...
const ProfileForm = ({
  ...
  const fields = [
    ...
    // Pickers (professional_type, military_branch) should always show if it's an admin view,
    ...
    if (field.type === 'picker' && (field.key === 'professional_type' || field.key === 'military_branch')) {
        if (isAdmin) return true; // Admins see all pickers
        if (isEditable) { // If editable, respect the user-toggle state
            if (field.dependsOnPickerToggle === 'user_professional_toggle' && showProfessionalPicker) return true;
            if (field.dependsOnPickerToggle === 'user_military_toggle' && showMilitaryPicker) return true;
            return false; // Hide pickers if editable but toggle is off
        }
        // If not editable (view-only mode), show if there's a value
        return !!initialData[field.key];
    }

    return true; // All other fields (text inputs, general pickers) are shown if in fieldsToShow
  });
